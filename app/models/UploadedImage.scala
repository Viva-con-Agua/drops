...
case class UploadedImage(uuid: UUID, name: String, contentType : String, base64: String, bufferedImage : BufferedImage, thumbnails : List[UploadedImage], selected: Boolean) {
...
object UploadedImage {
  def apply(selected: Boolean, name: String, contentType : String, base64: String): UploadedImage = {
    ...
    UploadedImage(UUID.randomUUID(), name, contentType, base64, image, Nil, selected)
  }

  def apply(selected: Boolean, file: File, name: Option[String], contentType: Option[String] = None): UploadedImage = {
    ...
    UploadedImage(UUID.randomUUID(), n, ct, base64, bufferedImage, Nil, selected)
  }

  def apply(uuid: UUID, selected: Boolean, name: String, contentType: String, bufferedImage : BufferedImage): UploadedImage = {
    val format = contentType.split("/").lastOption.getOrElse("png")
    val os = new ByteArrayOutputStream
    ImageIO.write(bufferedImage, format, os)
    val base64 = Base64.getEncoder().encodeToString(os.toByteArray())

    UploadedImage(uuid, name, contentType, base64, bufferedImage, Nil, selected)
  }
}
...
case class RESTImageResponse(url: String, id: UUID, selected: Boolean, width: Int, height: Int, thumbnails: List[RESTImageThumbnailResponse])
object RESTImageResponse {
  def apply(uploadedImage: UploadedImage): RESTImageResponse = {
    ...
    RESTImageResponse(
      url, uploadedImage.getID, uploadedImage.selected, uploadedImage.width, uploadedImage.height,
      uploadedImage.thumbnails.map(RESTImageThumbnailResponse(_, uploadedImage.getID))
    )
  }

  def apply(t: (String, UUID, Boolean, Int, Int, List[RESTImageThumbnailResponse])): RESTImageResponse =
    RESTImageResponse(t._1, t._2, t._3, t._4, t._5, t._6)

  implicit val restImageWrites : OWrites[RESTImageResponse] = (
    (JsPath \ "url").write[String] and
      (JsPath \ "id").write[UUID] and
      (JsPath \ "selected").write[Boolean] and
      (JsPath \ "width").write[Int] and
  ...
  implicit val restImageReads : Reads[RESTImageResponse] = (
    (JsPath \ "url").read[String] and
      (JsPath \ "id").read[UUID] and
      (JsPath \ "selected").read[Boolean] and
      (JsPath \ "width").read[Int] and
