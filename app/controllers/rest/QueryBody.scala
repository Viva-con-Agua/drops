...
object QueryBodyCrews {
  implicit val queryBodyCrewsFormat = Json.format[QueryBodyCrews]

  case class NoValuesGiven(msg: String) extends Exception(msg)
  case class NoQueryGiven(msg: String) extends Exception(msg)

  private def asRequest(body: QueryBodyCrews, view: String, ignorePagination : Boolean = false)(implicit messages: Messages) : Either[Exception, Converter] = {
    val page = if(!ignorePagination) body.limit.map((l) => Page(l, body.offset)) else None
  ...
  def asCrewsQuery(body: QueryBodyCrews)(implicit messages: Messages) : Either[Exception, Converter] = asRequest(body, "Crews")
  def asCrewsCountQuery(body: QueryBodyCrews)(implicit messages: Messages) : Either[Exception, Converter] = asRequest(body, "Crews", true)
}
